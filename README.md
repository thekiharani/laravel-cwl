...
```bash
gh repo create thekiharani/laravel-cwl --public --source=. --push
```

2. Go to [packagist.org/packages/submit](https://packagist.org/packages/submit) and paste the repo URL:

```
https://github.com/thekiharani/laravel-cwl
```

3. Click **Submit**. Packagist will index the package and pick up existing tags.

### Automated releases via GitHub Actions

Packagist needs to be notified when you push new tags. There are two ways:

**Option A: Packagist webhook (recommended, zero config)**

1. On Packagist, go to your package page and click **Settings**
2. Copy the **API Token** shown there
3. On GitHub, go to **Settings > Webhooks > Add webhook**
   - Payload URL: `https://packagist.org/api/github?username=thekiharani`
   - Content type: `application/json`
   - Secret: your Packagist API token
   - Events: select **Just the push event**
4. Done. Every `git push --tags` auto-updates Packagist.

**Option B: GitHub Actions workflow (if you also want CI)**
      ...
      - name: Update Packagist
        run: |
          curl -XPOST \
            ...
            'https://packagist.org/api/update-package?username=thekiharani&apiToken=${{ secrets.PACKAGIST_TOKEN }}' \
            -d '{"repository":{"url":"https://github.com/thekiharani/laravel-cwl"}}'
```

Then add your Packagist API token as a GitHub secret:

1. Go to [packagist.org/profile](https://packagist.org/profile/) and copy your API token
...
Packagist picks up the new tag automatically (via webhook or GHA). Users get it with:

```bash
composer update thekiharani/laravel-cwl
```
